...
* [AutoAuth](https://github.com/Swevo/AutoAuth) - A free, MIT-licensed fluent configuration wrapper around OpenIddict for building OAuth2/OIDC token servers in ASP.NET Core.
...
* [AutoArchitecture](https://github.com/Swevo/AutoArchitecture) - Free, MIT-licensed compile-time architecture rule enforcement for .NET, generated by a Roslyn source generator.
* [PollyAnalyzers](https://github.com/Swevo/PollyAnalyzers) - Free, MIT-licensed Roslyn analyzers for async/resilience anti-patterns.
...
* [AutoGuard](https://github.com/Swevo/AutoGuard) - Compile-time guard clauses for .NET, generated by a Roslyn source generator.
...
* [AutoFeatureFlag](https://github.com/Swevo/AutoFeatureFlag) - Compile-time feature flag stubs for .NET. Decorate an `enum` with `[FeatureFlags]` and get a strongly-typed interface and class — no reflection, no runtime overhead, AOT-safe.
...
* [AutoResult](https://github.com/Swevo/AutoResult) - A Roslyn source generator that automatically wraps your methods in `Result<T>` — zero reflection, zero runtime overhead, compile-time safe.
...
* [AutoHttpClient.Generator](https://github.com/Swevo/AutoHttpClient.Generator) - AOT-safe, compile-time typed HTTP client for .NET. Annotate an interface with `[HttpClient]`, decorate methods with `[Get]`, `[Post]`, `[Put]`, `[Delete]`, or `[Patch]`, and the generator emits a strongly-typed implementation plus DI registration at build time.
* [HttpClient.TestKit](https://github.com/Swevo/HttpClient.TestKit) - Unit-test `HttpClient` calls without mocking frameworks. Fluent setup, JSON responses, request recording, and built-in verification. Zero dependencies.
...
* [AutoImage](https://github.com/Swevo/AutoImage) - Free, MIT-licensed fluent image processing for .NET.
...
* [AutoLog.Generator](https://github.com/Swevo/AutoLog.Generator) - Turns simple `[Log]`-annotated partial methods into high-performance `LoggerMessage.Define` logging at build time. Generates cached delegates, `EventId`s, and partial method bodies.
...
* [Polly.Contrib.Bulkhead](https://github.com/Swevo/Polly.Contrib.Bulkhead) - Bulkhead isolation strategy for Polly v8 resilience pipelines. Limits concurrent executions and queued actions, rejecting excess calls with `BulkheadRejectedException`.
* [Polly.Contrib.Caching](https://github.com/Swevo/Polly.Contrib.Caching) - A caching resilience strategy for Polly v8 pipelines. Restores the beloved Polly v7 caching policy for the new `DelayGenerator`-based API.
* [Polly.Contrib.OpenTelemetry](https://github.com/Swevo/Polly.Contrib.OpenTelemetry) - OpenTelemetry instrumentation for Polly v8 resilience pipelines.
* [PollyAzureEventHub](https://github.com/Swevo/PollyAzureEventHub) - Polly v8 resilience for `Azure.Messaging.EventHubs` — add retry, timeout, and circuit-breaker to any Event Hubs producer operation in two lines.
* [PollyAzureKeyVault](https://github.com/Swevo/PollyAzureKeyVault) - Polly v8 resilience for `Azure.Security.KeyVault.Secrets` — add retry, timeout, and circuit-breaker to any Azure Key Vault operation in two lines.
* [PollyAzureQueueStorage](https://github.com/Swevo/PollyAzureQueueStorage) - Polly v8 resilience pipelines for Azure Queue Storage — retry, timeout, and circuit-breaker for QueueClient operations.
* [PollyAzureTableStorage](https://github.com/Swevo/PollyAzureTableStorage) - Polly v8 resilience for Azure.Data.Tables — add retry, timeout, and circuit-breaker to any Table Storage operation in two lines.
* [PollyElasticsearch](https://github.com/Swevo/PollyElasticsearch) - Polly v8 resilience for `Elastic.Clients.Elasticsearch 8+` — add retry, timeout, and circuit-breaker to any Elasticsearch operation in two lines.
* [PollyHangfire](https://github.com/Swevo/PollyHangfire) - Polly v8 resilience pipelines for Hangfire — retry, timeout, and circuit-breaker for IBackgroundJobClient.Enqueue and Schedule.
* [PollyMailKit](https://github.com/Swevo/PollyMailKit) - Polly v8 resilience pipelines for MailKit SMTP — retry, timeout, and circuit-breaker for sending email via SmtpClient.
* [PollyMassTransit](https://github.com/Swevo/PollyMassTransit) - Polly v8 producer-side resilience for MassTransit — add retry, timeout, and circuit-breaker to `IBus.Publish` and `SendAsync` in two lines.
* [PollyRateLimiter](https://github.com/Swevo/PollyRateLimiter) - Convenience extension methods for Polly v8 resilience pipelines.
* [PollySendGrid](https://github.com/Swevo/PollySendGrid) - Polly v8 resilience for SendGrid — add retry, timeout, and circuit-breaker to any email send in two lines.
* [AutoValidate.Generator](https://github.com/Swevo/AutoValidate.Generator) - Compile-time FluentValidation wiring for .NET, generated by a Roslyn source generator.
* [AutoQuery.Generator](https://github.com/Swevo/AutoQuery.Generator) - Compile-time query object generation for .NET using Roslyn source generators.
* [AutoAudit](https://github.com/Swevo/AutoAudit) - Compile-time audit field generation for EF Core entities. Add `[Auditable]` to any `partial` entity class and get `CreatedAt`, `UpdatedAt`, `CreatedBy`, and `UpdatedBy` properties plus a ready-to-use `AuditInterceptor` — all generated at build time.
...
* [FluentExcel](https://github.com/Swevo/FluentExcel) - Free, MIT-licensed fluent Excel generation for .NET.
...
* [EFCore.JsonColumn](https://github.com/Swevo/EFCore.JsonColumn) - Compile-time JSON column configuration for EF Core 8+. Stamp `[JsonColumn]` on owned navigation properties and the source generator emits `ConfigureJsonColumns(ModelBuilder)`.
* [EFCore.MultiTenant](https://github.com/Swevo/EFCore.MultiTenant) - Compile-time multi-tenancy for EF Core. Stamp a `[Tenant]` attribute on any entity and the source generator wires up `TenantId`, a save interceptor, and global query filters automatically.
* [EFCore.Outbox](https://github.com/Swevo/EFCore.Outbox) - Transactional outbox pattern for EF Core. Enqueue domain events inside your existing `SaveChanges` transaction and publish them reliably via a background processor.
* [EFCore.Pagination](https://github.com/Swevo/EFCore.Pagination) - Offset and cursor-based pagination for EF Core. Two extension methods on `IQueryable<T>`, two result types.
* [EFCore.RowVersion](https://github.com/Swevo/EFCore.RowVersion) - Compile-time optimistic concurrency for EF Core. Stamp `[Optimistic]` on any partial entity class and the source generator wires up a `RowVersion` byte array property and retry extensions for concurrent-write conflicts.
* [EFCore.Seeding](https://github.com/Swevo/EFCore.Seeding) - Fluent, idempotent, dependency-ordered seed data for EF Core. Define `IEntitySeed<T>` classes, declare run-order dependencies, then call `SeedDatabaseAsync()` on startup.
* [EFCore.SoftDelete](https://github.com/Swevo/EFCore.SoftDelete) - Compile-time soft-delete generation for EF Core entities. Add `[SoftDelete]` to any `partial` entity class and get `IsDeleted` + `DeletedAt` fields, an interceptor, and a global query filter — all at build time.
* [EFCore.StronglyTyped](https://github.com/Swevo/EFCore.StronglyTyped) - Compile-time strongly-typed ID generation for .NET using Roslyn source generators.
...
* [FluentPdf](https://github.com/Swevo/FluentPdf) - Free, MIT-licensed fluent PDF generation for .NET.
...
* [AutoBus](https://github.com/Swevo/AutoBus) - Free, MIT-licensed message bus for .NET.
* [MassTransit.RequestClient.TestKit](https://github.com/Swevo/MassTransit.RequestClient.TestKit) - Lightweight test doubles for MassTransit request/response patterns. Drop-in fakes for `IRequestClient<T>` and `IPublishEndpoint` — no harness, no bus, no infrastructure.
## RPC
...
* [AutoAssert](https://github.com/Swevo/AutoAssert) - Free, MIT-licensed fluent assertions for .NET.
* [AutoTestData](https://github.com/Swevo/AutoTestData) - Compile-time test data builders for .NET, generated by a Roslyn incremental source generator. An AutoFixture/Bogus alternative with zero reflection and full AOT/trim support.
## Tools
